...
(./apktool d original.apk -q && rm ./original/assets/Mili_wuhan.* && cp ../../MiBand3Firmwares/$FIRMWARE_VERSION/Mili_wuhan.* './original/assets' &&\
 rm ./original/assets/firmwares.json && cp ../../MiBand3Firmwares/$FIRMWARE_VERSION/firmwares.json ./original/assets/firmwares.json &&\
 ./apktool b original -q -o modified.apk) || exception '변조 및 다시 빌드하지 못하였습니다.'
